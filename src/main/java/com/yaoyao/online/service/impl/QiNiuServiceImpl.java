...
public class QiNiuServiceImpl implements IQiNiuService, InitializingBean {
    ...
    public Response uploadFile(File file) throws QiniuException {
        Response response = this.uploadManager.put(file, null, uploadToken());
        int retry = 0;
        while(retry < 3 && response.needRetry()){
            response = this.uploadManager.put(file, null, uploadToken());
        }
        return response;
    }
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        putPolicy.put("returnBody", "{\"key\":\"$(key)\",\"hash\":\"$(etag)\",\"bucket\":\"$" +
                "(bucket)\",\"width\":$(imageInfo.width),\"height\":$(imageInfo.height)}");
